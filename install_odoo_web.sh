...
# Installation Odoo Web Server (sans PostgreSQL local)
set -e

# Variables de la base de données (injectées par Terraform)
DB_HOST="${db_host}"
DB_PORT="${db_port}"
DB_NAME="${db_name}"
DB_USER="${db_user}"
...
# Install system dependencies (sans PostgreSQL)
...
# Install PostgreSQL client only (pour se connecter au RDS)
...
echo "📁 Création du répertoire addons..."
...
# Create Odoo config with RDS connection
echo "⚙️ Configuration d'Odoo avec RDS..."
...
until PGPASSWORD="$DB_PASSWORD" psql -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" -d "$DB_NAME" -c "SELECT version();" > /dev/null 2>&1; do
    echo "⏳ Attente de la disponibilité de RDS..."
    sleep 10
done
echo "✅ Connexion RDS établie!"

# Enable PostGIS on RDS (si pas déjà fait)
echo "🗺️ Activation de PostGIS sur RDS..."
PGPASSWORD="$DB_PASSWORD" psql -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" -d "$DB_NAME" -c "CREATE EXTENSION IF NOT EXISTS postgis;" || echo "PostGIS déjà installé"
...
tee /opt/odoo/webhook.py > /dev/null <<'EOF'
#!/usr/bin/env python3
import json
import subprocess
import os
...
systemctl status odoo.service --no-pager -l || true
systemctl status odoo-webhook.service --no-pager -l || true

echo ""
echo "🎉 Installation terminée!"
echo "📋 Odoo: http://$(curl -s ifconfig.me):8069"
echo "🔄 Webhook: http://$(curl -s ifconfig.me):9000/deploy"
echo "🗄️ Base de données: $DB_HOST:$DB_PORT"
